...
public interface TopManagerMapper extends BaseMapper<User> {
    @Select("SELECT * FROM user WHERE userId = #{num} and status >= 10")
    ...
    @Update("UPDATE user SET password = #{password}, modifyTime = #{modifyTime} WHERE userId = #{userId} and status >= 10")
    int updatePassword(@Param("userId") String userId, @Param("password") String password, @Param("modifyTime") Date modifyTime);

    @Update("UPDATE `user` SET phone = #{phone}, modifyTime = #{modifyTime} WHERE userId = #{userId} and status >= 10")
    int updatePhone(@Param("userId") String userId, @Param("phone") String phone, @Param("modifyTime") Date modifyTime);

    @Update("UPDATE `user` SET description = #{description}, modifyTime = #{modifyTime} WHERE userId = #{userId} and status >= 10")
