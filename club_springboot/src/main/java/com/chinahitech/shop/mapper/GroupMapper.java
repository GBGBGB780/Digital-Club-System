...
public interface GroupMapper extends BaseMapper<Group> {
    ...
    @Update("UPDATE `group` SET attachment = #{attachment}, modify_time = #{modifyTime} WHERE name = #{name} and is_accepted = true")
    ...
    @Select("select * from `group` WHERE is_accepted = false")
    List<Group> findAllApp();

    @Select("SELECT * FROM `group` WHERE name LIKE CONCAT('%', #{searchinfo}, '%') and is_accepted = false")
    List<Group> findAppBySearch(String searchinfo);

    @Select("SELECT * FROM `group` WHERE name = #{groupname} and is_accepted = false")
